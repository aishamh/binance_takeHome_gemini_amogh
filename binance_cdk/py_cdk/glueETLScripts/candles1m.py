...
def rename_s3() -> None:
 ...
 binance_candles1m_datafeed = glueContext.create_dynamic_frame.from_catalog(database = "binance_raw_data", table_name = "binance_candles1m_datafeed", additional_options = {"recurse": True}, push_down_predicate = pdp ,transformation_ctx = "binance_candles1m_datafeed")


 binance_candles1m_datafeed.coalesce(1).write.format('parquet').mode("append").option("compression", "snappy").partitionBy("dataset_date").save(s3_dest , header = 'true')
